# Input File
data.germany <- read.csv(file="data.csv")
############################
...
holder <- matrix(NA, nrow= ncol(data.germany.ibs), ncol=ncol(data.germany.ibs),
...
write.csv(data.germany.ibs.similarity,file="final-similarity.csv")
...
for(i in 1:ncol(data.germany.ibs))
{
  data.germany.neighbours[i,] <- (t(head(n=11, rownames(data.germany.ibs.similarity
                                                        [order(data.germany.ibs.similarity[,i],decreasing=TRUE),][i]))))
}
# Output neighbour results to a file
write.csv(file="final-item-neighbours.csv",x=data.germany.neighbours[,-1])
############################
# User Scores Matrix #
############################
# Process:
...
holder <- matrix(NA, nrow=nrow(data.germany),ncol=ncol(data.germany)-1,
...
for(i in 1:nrow(holder))
{
  ...
  for(j in 1:ncol(holder))
  {
    ...
    if(as.integer(data.germany[data.germany$user==user,product]) == 1)
    {
      ...
      topN<-
        ((head(n=11,(data.germany.ibs.similarity[order(data.germany.ibs.similarity[,product],decreasing=TRU
                                                       E),][product]))))
      ...
      topN.userPurchases <- as.numeric(topN.userPurchases[!(names(topN.userPurchases) %in%
                                                              c("user"))])
...
for(i in 1:nrow(data.germany.user.scores))
{
  data.germany.user.scores.holder[i,] <-
    names(head(n=100,(data.germany.user.scores[,order(data.germany.user.scores[i,],decreasing=TRUE)])[
      i,]))
}
# Write output to file
write.csv(file="final-user-recommendations.csv",data.germany.user.scores.holder)
impordata <- read.csv("final-user-recommendations.csv")
readinteger <- function()
{
  n <- readline(prompt="Enter an Book ID: ")
  if (is.na(n)){
    n <- readinteger()
  }
  return(n)
...
u <- readinteger()
impordata <- subset(impordata,impordata$Users==u)
